...
import { Shuffle } from '../data/blog/advent-of-javascript/day-4/Shuffle'
...
1. Next, I wanted to see if there's an official algorithm to what I proposed above, and there is: [Fisher-Yates Shuffle](https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle)
   ...
   1. Using the previous swap algorithm, we swap the randomly selected key with the unshuffled key at the end.
   1. After we finish shuffling and the `end` is `0`, we reset it back to `keys.length` & reshuffle the shuffled array all over again.

## Demo

I spent the most time actually writing this blog post and porting over [Mike Bostock's visualization](https://bost.ocks.org/mike/shuffle/) :)
