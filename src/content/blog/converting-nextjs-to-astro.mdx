...
- [Spotlight](https://tailwindui.com/templates/spotlight) TailwindCSS Theme
- [TailwindCSS Support](https://docs.astro.build/en/guides/integrations-guide/tailwind/)
...
The fact that Astro **shows you what changed** is a great DX. Most CLIs will modify `package.json` & configs, but it's up to you to recognize the changes the next time you run `git add -p`.
...
When I'm missing a dependency, such as [clsx](https://github.com/lukeed/clsx), the build predicatably fails. **But**, if I install it in another terminal window, Astro _rebuilds_ the page without anything else from me!

### `<Image />` doesn't work within React components, only `.astro` components

[Improve error handling for @astro/image within react components](https://github.com/withastro/astro/issues/5673#top)
...
Where I struggled with Qwik was getting a list of all the content on the filesystem.
...
However, I'm interested in trying Astro's [Content Collections (Experimental)](https://docs.astro.build/en/guides/content-collections/) so that frontmatter is strictly enforced, there can be different sub-sections of content, and content is separate from the theme.

A couple things immediately impressed me!
...
Second, `getCollection` is _automatically typed_ with the **actual data**. This means that TypeScript knows what posts exist, what their slugs are, and all of their meta data.
...
I'm using `.mdx` everywhere, so `getEntry('blog', 'javascript-fatigue.mdx')` isn't much a problem, but it feels like there should be a direct equivalent between URL's `slug` & the equivalent on the filesystem without having TypeScript errors:
...
I expected that at least `h1`, `h2`, `p`, etc. would work (not actualy _components_) with `<Content components={...} />`, but no.

### Converting from React to `.astro`

What I've found is that presentational (read: logicless) components are pretty straightforward to convert.
...
But I found it went smoothest by aliasing `react` & `react-dom` to `@preact/compat` rather than relying on [pnpm.overrides](https://pnpm.io/package_json#pnpmoverrides) (which didn't work for me).
...
In practice, `<Picture>` is most analagous to [next/image](https://nextjs.org/docs/api-reference/next/image) for **responsive images** because it also supports the [sizes](https://developer.mozilla.org/en-US/docs/Web/API/HTMLImageElement/sizes) attribute.

The only difficult is also providing [widths](https://docs.astro.build/en/guides/integrations-guide/image/#widths) in pixels when Tailwind's units are amibiguous – `w-72` might mean `18rem`, or `288px` ¯\_(ツ)\_/¯
...
I typically set a maximum `width` of `1024` and an `aspectRatio` of `16:9`.

But what makes these so handy is that I can use an `import` statement for the `src`!
...
And finally create an [ArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer) by:
...
But the probem is, even when I get this working locally, is that Vercel has cryptic errors that I have no way of diagnosing:
...
1. Astro **feels like the old days of the web.**
   ...
   It feels like I start with _form_, then get to choose how to progressively add _function_. [Remix](https://remix.run/) may say the same, as it centralizes on primatives like Request & Response. But being React-first rather than _HTML_-first means abstractions for `link`/`style`/`script` are essential.

2. Astro's growth and positive impressions are warranted, & will only improve:
   ![StateOfJS](https://assets.devographics.com/captures/js2022/en-US/astro.png)
