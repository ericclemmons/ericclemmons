...
I've been a big fan of [GitButler](https://gitbutler.com/) for a few years now –
...
- PR #2: Feature A (`pr-split/eric/feature-a`)
  This PR is automatically created by AI based on changes in the ☂️ that AI determines can be landed independently to `main`.
...
1. <kbd>↑</kbd> and <kbd>↓</kbd> to highlight a commit and <kbd>Enter</kbd> to *Assign**
   it to the current PR
1. <kbd>←</kbd> and <kbd>→</kbd> to navigate between available PRs{' '}
...
Now, I realize my mistake – **this works best for a _stack_ of dependent PRs, not for independent PRs** like I wanted with my AI experiment.
