  ...
  if [ "$ACTION" = "create" ]; then
    ...
    if [ -z "$PR_NUMBER" ]; then
      ...
      if [ -n "$EXISTING_PR_NOW" ]; then
        ...
        # Create new PR with timeout to prevent hanging
        echo "  🔨 Creating PR..."
        set +e
        
        # Use timeout if available (30 seconds should be plenty)
        if command -v timeout &> /dev/null; then
          PR_URL=$(timeout 30 gh pr create \
            ...
            --head "$branch" \
            --title "$TITLE" \
            --body "Auto-generated by PR Splitter
...
$REASONING" 2>&1)
        ...
        if [ $PR_CREATE_EXIT -eq 0 ]; then
          ...
          # Check if PR was created anyway
          sleep 2
          PR_NUMBER=$(gh pr list --head "$branch" --state all --json number --jq '.[0].number' 2>/dev/null || echo "")
          if [ -n "$PR_NUMBER" ]; then
            echo "  ✅ Found PR #$PR_NUMBER (created despite timeout)"
          else
            echo "  ⚠️  No PR found - branch pushed but PR creation failed"
            echo "  Branch: $branch exists and can be viewed on GitHub"
          fi
        else
          # Other error
          if echo "$PR_URL" | grep -q "does not have permission to update"; then
            echo "  ℹ️  PR exists but created by different user"
            PR_NUMBER=$(gh pr list --head "$branch" --json number --jq '.[0].number' 2>/dev/null || echo "")
            if [ -n "$PR_NUMBER" ]; then
              echo "  ✅ Found existing PR #$PR_NUMBER"
            fi
          else
            echo "  ⚠️  Failed to create PR (exit code: $PR_CREATE_EXIT)"
            if [ -n "$PR_URL" ]; then
              echo "  Error: $PR_URL"
            fi
            # Check if PR exists anyway
            PR_NUMBER=$(gh pr list --head "$branch" --state all --json number --jq '.[0].number' 2>/dev/null || echo "")
            if [ -n "$PR_NUMBER" ]; then
              echo "  ✅ Found existing PR #$PR_NUMBER"
            else
              echo "  ℹ️  Branch pushed successfully, PR can be created manually"
            fi
          fi
        fi
      fi
